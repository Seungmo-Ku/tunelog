...
export const useGetCommunityItems = (limit: number = 10, type: CommunityQueryType = 'all', sort: CommunitySortType = 'newest', filter: 'all' | 'following' = 'all') => {
    const { me } = useAccount()
    return useInfiniteQuery<DataConnection<CommunityResponse>, Error>({
        queryKey: ['community-all', limit, type, sort, filter, me?._id ?? ''],
        queryFn: async ({ pageParam }) => {
            const cursor = typeof pageParam === 'string' ? pageParam : ''
            return await ApiCommunity._get_community_items(limit, type, sort, filter, cursor) ?? { data: [], nextCursor: undefined }
        },
        initialPageParam: '',
